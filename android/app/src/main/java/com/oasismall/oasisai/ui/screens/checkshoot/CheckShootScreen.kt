...
fun CheckShootScreen(
    ...
    LaunchedEffect(prefillBarcode, startCapture, isLocked, scan, captureLaunched) {
        if (startCapture && !captureLaunched && isLocked && scan != null && phase == CheckShootPhase.SCANNING) {
            ...
            viewModel.prepareCreateAsset { launchCamera() }
        }
    }
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    androidx.compose.material3.Button(
                        onClick = viewModel::confirmSubBarcodeAdd,
    ...
    Column(Modifier.fillMaxSize()) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                Text(
                    "Scan barcode → lock → share or shoot PNG",
                    color = Color.White.copy(alpha = 0.85f),
                    style = MaterialTheme.typography.labelSmall,
                )
            }
            if (phase == CheckShootPhase.SCANNING && suffixMatch == null) {
        ...
        Box(Modifier.weight(1f).fillMaxWidth()) {
            if (hasCameraPermission && phase == CheckShootPhase.SCANNING) {
                BarcodeCameraPreview(
                    enabled = true,
                    barcodeScanEnabled = (!isLocked || subBcMode) && suffixMatch == null,
            ...
            if (scan == null && phase == CheckShootPhase.SCANNING && hasCameraPermission) {
                Surface(
                    ...
                    Text(
                        "Scan barcode → card shows details → hold card 1s to lock → choose action → swipe to unlock",
...
private fun CheckShootArticlePopup(
    ...
    Column(modifier = modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(6.dp)) {
        Text(
            if (isLocked) {
                "Glissez la carte ← ou → pour le scan suivant"
            } else {
                "Glissez ← ou → pour fermer · maintenez 1 s (ou cadenas) pour verrouiller"
            },
            style = MaterialTheme.typography.labelMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
            modifier = Modifier.padding(horizontal = 4.dp),
        )
        onLinkToCatalog?.let { link ->
            OutlinedButton(onClick = link, modifier = Modifier.fillMaxWidth()) {
                Text("Link barcode to catalog article")
            }
        }
        Box(
            modifier = Modifier
